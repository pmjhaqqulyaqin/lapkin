      ...
      {isOpen && createPortal(
        <div className="fixed inset-0 z-[999] flex flex-col justify-end">
          ...
          >
            ...
            {/* Header */}
            <div className="px-4 pb-2.5 flex justify-between items-center border-b border-slate-100 dark:border-slate-800">
              <h3 className="font-manrope font-bold text-[14px] text-slate-800 dark:text-slate-100">
                {title}
              </h3>
              <button 
                onClick={() => setIsOpen(false)} 
                className="w-7 h-7 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full flex items-center justify-center text-slate-500 transition-colors"
              >
              ...
              <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg pl-8 pr-4 py-2 text-[12px] text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-teal-500/50 outline-none transition-all"
                  />
                  {searchQuery && (
                    <button 
            ...
            <div className="flex-1 overflow-y-auto px-2 py-2 scroll-smooth">
              ...
              {enableRecent && !searchQuery && recentItems.length > 0 && (
                <div className="mb-2">
                  <p className="px-2 py-1.5 text-[9px] font-bold text-slate-400 uppercase tracking-widest">Sering Digunakan</p>
                  {recentItems.map(item => (
                    <button
                      key={`recent-${item}`}
                      onClick={() => handleSelect(item)}
                      className="w-full text-left px-2.5 py-2.5 text-[12px] font-semibold text-teal-700 dark:text-teal-400 bg-teal-50/50 dark:bg-teal-900/10 hover:bg-teal-50 dark:hover:bg-teal-900/30 rounded-lg transition-colors flex items-center gap-2 mb-0.5 border border-teal-100 dark:border-teal-900/50"
                    >
                      <span className="material-symbols-outlined text-[16px]">history</span>
                      <span className="truncate">{item}</span>
                    </button>
                  ))}
                  <div className="h-px bg-slate-100 dark:bg-slate-800/50 my-1.5 mx-2" />
                </div>
              )}

              {/* All Options Section */}
              {enableRecent && !searchQuery && (
                <p className="px-2 py-1.5 text-[9px] font-bold text-slate-400 uppercase tracking-widest">Semua Opsi</p>
              )}
              
              {filteredOptions.length === 0 ? (
                <div className="px-4 py-8 flex flex-col items-center text-center">
                  <span className="material-symbols-outlined text-[32px] text-slate-300 dark:text-slate-700 mb-2">search_off</span>
                  <p className="text-slate-500 text-[12px] font-medium">Tidak ada hasil ditemukan.</p>
                </div>
              ) : (
                filteredOptions.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-2.5 py-2.5 my-0.5 rounded-lg text-[12px] transition-colors flex items-center gap-2 ${
                        isSelected 
                    ...
                    >
                      {isSelected ? (
                        ...
                        <span className="material-symbols-outlined text-[18px] text-slate-300 dark:text-slate-700">radio_button_unchecked</span>
                      )}
